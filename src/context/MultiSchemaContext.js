...
export const MultiSchemaProvider = ({ children }) => {
  ...
  const getSchemaState = useCallback((schemaId) => {
    return schemaStates[schemaId] || createDefaultSchemaState();
  }, [schemaStates]);

  // Update state for a specific schema
  const updateSchemaState = useCallback((schemaId, updates) => {
  ...
  const getDeletedAttributes = useCallback((schemaId) => {
  ...
  const initializeSchemaFromOCA = useCallback((schemaId, ocaPackage) => {
    ...
    const normalizeType = (rawType) => {
      ...
      if (Array.isArray(rawType)) {
        ...
        if (typeof first === 'string') {
          ...
          if (inner.startsWith('refn:') || inner.startsWith('refs:')) return 'Array[Child Schema]';
          const mapped = {
            text: 'Text',
            numeric: 'Numeric',
            boolean: 'Boolean',
      ...
      if (arrayMatch) {
        ...
        if (inner.toLowerCase().startsWith('refn:') || inner.toLowerCase().startsWith('refs:')) return 'Array[Child Schema]';
        const mapped = {
          text: 'Text',
          numeric: 'Numeric',
          boolean: 'Boolean',
    ...
    // Build attributes
    const attributes = Object.entries(schemaData.attributes || {}).map(([name, type]) => ({
      Attribute: name,
    ...
    const addEntry = (attrName, code, lang, text) => {
      ...
      if (!row) { row = { Code: code }; entryCodes[attrName].push(row); }
      row[lang] = text || '';
    };
    ...
    if (Array.isArray(entryOverlay)) {
      entryOverlay.forEach((o) => {
        ...
        Object.entries(attrMap).forEach(([attr, codeMap]) => {
          Object.entries(codeMap || {}).forEach(([code, text]) => addEntry(attr, code, lang, text));
        });
      });
    ...
    if (charEncodingOverlay?.attribute_character_encoding) {
      Object.entries(charEncodingOverlay.attribute_character_encoding).forEach(([attr, encoding]) => {
    ...
    if (cardinalityOverlay?.attribute_cardinality) {
      Object.entries(cardinalityOverlay.attribute_cardinality).forEach(([attr, cardinality]) => {
    ...
    if (conformanceOverlay?.attribute_conformance) {
      Object.entries(conformanceOverlay.attribute_conformance).forEach(([attr, conformance]) => {
    ...
    const newState = {
      ...
      entryCodes,
      attributesWithLists: attributesWithLists.filter(a => a.List).map(a => a.Attribute),
      lanAttributeRowData,
      characterEncodingData,
      formatRuleData,
      cardinalityData,
  ...
  const switchToSchema = useCallback((schemaId, ocaPackage) => {
    ...
    const canonicalizeSchemaId = (pkg, id) => {
      ...
      if (Array.isArray(metaOverlay)) {
        ...
        Object.keys(metaOverlay).forEach((lang) => {
          ...
          if (metaForLang && typeof metaForLang === 'object' && typeof metaForLang.name === 'string') {
    ...
    setSchemaStates(prev => {
      const willMigrate = schemaId !== resolvedId && prev[schemaId] && !prev[resolvedId];
  ...
  const isSchemaModified = useCallback((schemaId) => {
    return modifiedSchemas.has(schemaId);
  }, [modifiedSchemas]);

  // Get all modified schemas
  const getModifiedSchemas = useCallback(() => {
    return Array.from(modifiedSchemas);
  }, [modifiedSchemas]);

  // Export schema changes back to OCA package format
  const exportSchemaChanges = useCallback((ocaPackage) => {
  ...
  const getNavigationHistory = useCallback(() => {
    return [...schemaNavigationHistory];
  ...
  const saveToLocalStorage = useCallback(() => {
    ...
    try {
      localStorage.setItem(`oca_composer_multischema_${currentPackageId}`, JSON.stringify(stateToSave));
